...
(define (queue-empty? q)
  (if (and (= 0 (queue-size-l q)) (= 0 (queue-size-r q)))
      #t
      #f))
...
(define (enqueue x q)
  (cond
    ((queue-empty? q) (make-queue (stream-cons x empty-stream) null 1 0))
    ((> (+ 1 (queue-size-r q)) (queue-size-l q)) (make-queue (rotate (queue-left q) (queue-right q) (stream-cons x empty-stream)) null (+ (queue-size-r q) (queue-size-l q) 1) 0))
    (else (make-queue (queue-left q) (cons x (queue-right q)) (queue-size-l q) (+ 1 (queue-size-r q))))))
...
(define (dequeue q)
  (cond
    ((> (queue-size-r q) (- (queue-size-l q) 1)) (make-queue (stream-rest (rotate (queue-left q) (queue-right q) empty-stream)) null (+ (queue-size-r q) (queue-size-l q) -1) 0))
